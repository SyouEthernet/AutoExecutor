...
while 1:
    ...
    # 极光推送相关配置
    jpushKey = config['jpush']['key']
    jpushSecret = config['jpush']['secret']
    ...
    try:
        ...
        result = worker.doWork(bmobApi=config['bmob']['apiId'], bmobKey=config['bmob']['restKey'])
    except Exception as e:
        # 此处处理异常问题，可以考虑用文件记录
        errorLogger.log(str(e))
    finally:
        # 最后的通用执行，在此处理
        # 正常的result通常都是返回0的，不为0，表示有需要执行的步骤没有执行
        # 由于执行数据的写入数据库操作可以在dowork中进行，这里可以只处理异常的
        # 或者也可以在这里上报,通过极光推送进行推送
        print(result)
        pusher.push(jpushKey, jpushSecret, 'test:' + str(result))
